...
import db from './selectQueries.js'
...
async function isValidToken(token) {
    if (token != undefined && typeof (token) == 'string' && token.trim() != '') {
        let header_sql = `Select token from public.team_table where token = '${token}'`
        let header_sql_res = await client.query(header_sql)
        console.log(header_sql_res.rows, "     ", token);
        if (header_sql_res.rows.length > 0) {
